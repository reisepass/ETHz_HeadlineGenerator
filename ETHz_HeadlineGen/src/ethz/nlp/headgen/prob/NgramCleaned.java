...
public class NgramCleaned extends NgramSimple implements NGramProbs {
	...
	// This class should only get used if the corpus it uses in the TreeMap was
	// also cleaned in the same way
	public TreeMap<ArrayList<String>, Double> filterNgrams(Doc doc) {
		FirstSentSum.removePoSNotInList(doc.getAno()
				.get(TokensAnnotation.class), FirstSentSum.CLEAN_WORDS);
		return super.filterNgrams(doc); // TODO make sure that
										// removePoSNotInList works as expected
	}
